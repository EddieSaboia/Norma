...
public class Norma {
	...
	public void acao() throws Exception {
		...
		for (int i = 0; i < instrucoes.size(); i++) {
			...
			Interpretador interpretador = instrucao.getComando();

			switch (interpretador) {
			case ISZERO:
	...
	public void execultarCondicaoIfElseWhile(Instrucao instrucao) throws Exception {
		...
		for (int i = 0; i < instrucoesInterna.size(); i++) {
			...
			if (instrucaoInterna.getComando() == Interpretador.IF) {
				checarCondicaoIfElse(instrucaoInterna);
			} else if (instrucaoInterna.getComando() == Interpretador.WHILE) {
				checarCondicaoWhile(instrucaoInterna);
			} else {

				if (instrucaoInterna.getComando() == Interpretador.ELSE)
					continue;
				if (instrucaoInterna.getComando() == Interpretador.GOTO) {
	...
	public Instrucao procurarInstrucaoElse(Instrucao instrucao) {
		...
		for (int i = 0; i < instrucoesInternaIf.size(); i++) {
			...
			if (instrucaoInternaIf.getComando() == Interpretador.ELSE) {
	...
	public void receberInstrucoes(String codigo) throws Exception {
		...
		for (int i = 0; i < instucoesString.size(); i++) {
			...
			if (inst != null) {
				if (inst.getComando() == Interpretador.IF || inst.getComando() == Interpretador.WHILE || inst.getComando() == Interpretador.ELSE) {
					pularBlocoIfWhile = 0;
	...
	public Instrucao gerarInstrucao(List<String> linha, List<String> instucoesString, int i) {
		...
		case "inc":
			linha.remove(0);
			return new Instrucao(Interpretador.INC, linha, null);

		case "dec":
			linha.remove(0);
			return new Instrucao(Interpretador.DEC, linha, null);

		case "set0":
			linha.remove(0);
			return new Instrucao(Interpretador.SETZERO, linha, null);

		case "is0":
			linha.remove(0);
			return new Instrucao(Interpretador.ISZERO, linha, null);

		case "goto":
			linha.remove(0);
			return new Instrucao(Interpretador.GOTO, linha, null);
		case "if":
		...
		case "set":
			linha.remove(0);
			return new Instrucao(Interpretador.SET, linha, null);
		case "add":
			linha.remove(0);
			return new Instrucao(Interpretador.ADD, linha, null);
		case ":":
			return new Instrucao(Interpretador.DOISPONTOS, linha, null);
	...
	public Instrucao gerarIntrucaoIfWhile(List<String> instucoesString, int i, List<String> linha) {
		...
		if (comando.equals("if")) {
			return new Instrucao(Interpretador.IF, linha, instrucoesInternas);
		} else if(comando.equals("while")) {
			return new Instrucao(Interpretador.WHILE, linha, instrucoesInternas);
		}else {
			return new Instrucao(Interpretador.ELSE, linha, instrucoesInternas);
		}
